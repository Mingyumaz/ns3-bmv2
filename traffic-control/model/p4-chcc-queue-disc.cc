 ...
 * Modified by:   Mingyu Ma <myma979@gmail.com>
 * 
 */
...
NS_LOG_COMPONENT_DEFINE ("P4ChccQueueDisc");

NS_OBJECT_ENSURE_REGISTERED (P4ChccQueueDisc);

// Initialize static members
Ptr<Packet> P4ChccQueueDisc::default_packet = Create<Packet> ();

TypeId P4ChccQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::P4ChccQueueDisc")
    .SetParent<QueueDisc> ()
    .SetGroupName ("TrafficControl")
    .AddConstructor<P4ChccQueueDisc> ()
...
std::string
P4ChccQueueDisc::GetJsonFile (void) const
{
  NS_LOG_FUNCTION (this);
  return m_jsonFile;
}
...
void
P4ChccQueueDisc::SetJsonFile (std::string jsonFile)
{
...
std::string
P4ChccQueueDisc::GetCommandsFile (void) const
{
  NS_LOG_FUNCTION (this);
...
void
P4ChccQueueDisc::SetCommandsFile (std::string commandsFile)
{
...
void
P4ChccQueueDisc::InitStdMeta (std_meta_t &std_meta)
{
  //
...
bool
P4ChccQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
...
P4ChccQueueDisc::RunTimerEvent ()
{
  ...
  m_timerEvent = Simulator::Schedule (m_timeReference, &P4ChccQueueDisc::RunTimerEvent, this);
}

void
P4ChccQueueDisc::RunDropEvent (Ptr<const QueueDiscItem> item)
{
  uint32_t nQueued = GetCurrentSize ().GetValue ();
  //
  // Initialize standard metadata
...
void
P4ChccQueueDisc::RunEnqEvent (Ptr<const QueueDiscItem> item)
{
  uint32_t nQueued = GetCurrentSize ().GetValue ();
  //
  // Initialize standard metadata
...
void
P4ChccQueueDisc::RunDeqEvent (Ptr<const QueueDiscItem> item)
{
  uint32_t nQueued = GetCurrentSize ().GetValue ();
  //
  // Initialize standard metadata
...
uint32_t
P4ChccQueueDisc::MapSize (double size)
{
...
void
P4ChccQueueDisc::InitializeParams (void)
{
  NS_LOG_FUNCTION (this);
...
double
P4ChccQueueDisc::Estimator (uint32_t nQueued, uint32_t m, double qAvg, double qW)
{
...
Ptr<QueueDiscItem>
P4ChccQueueDisc::DoDequeue (void)
{
  NS_LOG_FUNCTION (this);
...
bool
P4ChccQueueDisc::CheckConfig (void)
{
  NS_LOG_FUNCTION (this);
  if (GetNInternalQueues () > 0)
    {
      NS_LOG_ERROR ("P4ChccQueueDisc cannot have internal queues");
      return false;
    }

  if (GetNPacketFilters () > 0)
    {
      NS_LOG_ERROR ("P4ChccQueueDisc cannot have any packet filters");
      return false;
    }
  ...
  if (GetNQueueDiscClasses () == 0)
    {
      ...
      if (!qd->SetMaxSize (GetMaxSize ()))
        {
          NS_LOG_ERROR ("Cannot set the max size of the child queue disc to that of P4ChccQueueDisc");
          return false;
        }
      qd->Initialize ();
  ...
  if (GetNQueueDiscClasses () != 1)
    {
      NS_LOG_ERROR ("P4ChccQueueDisc requires exactly 1 class");
      return false;
    }

  if (m_jsonFile == "")
    {
      NS_LOG_ERROR ("P4ChccQueueDisc is not configured with a JSON file");
      return false;
    }

  if (m_commandsFile == "")
    {
      NS_LOG_ERROR ("P4ChccQueueDisc is not configured with a CLI commands file");
      return false;
    }
  ...
  if (m_enDropEvents)
    {
      TraceConnectWithoutContext ("DropBeforeEnqueue", MakeCallback (&P4ChccQueueDisc::RunDropEvent, this));
    }

  // Check if enqueue events are enabled
  if (m_enEnqEvents)
    {
      TraceConnectWithoutContext ("Enqueue", MakeCallback (&P4ChccQueueDisc::RunEnqEvent, this));
    }

  // Check if dequeue events are enabled
  if (m_enDeqEvents)
    {
      TraceConnectWithoutContext ("Dequeue", MakeCallback (&P4ChccQueueDisc::RunDeqEvent, this));
    }
